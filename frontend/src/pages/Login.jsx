...
import { BookOpen, ArrowRight, User, Lock, Loader2 } from 'lucide-react'; // Loader2 sudah diimport
...
const Login = () => {
    ...
    const handleLogin = async (e) => { // Fungsi diubah menjadi ASYNCHRONOUS
        ...
        try {
            ...
            setLoading(false);
            if (err.response) {
                // Tampilkan pesan error dari Laravel (401 Unauthorized atau pesan lain)
                setError(err.response.data.message || 'Login gagal. Cek kredensial Anda.');
            } else {
                 // Error koneksi (misal server Laravel mati)
    ...
    return (
        ...
        <div style={{
            ...
            <div className="card" style={{
                ...
                {/* LOGO */}
                <div style={{
                    background: '#0284c7', color: 'white', width: 56, height: 56, 
                    borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    margin: '0 auto 15px auto', boxShadow: '0 4px 10px rgba(2, 132, 199, 0.2)'
                }}>
                    <BookOpen size={28} />
                </div>
                ...
                {/* ERROR MESSAGE BAR */}
                {error && (
                    <div style={{backgroundColor: '#fef2f2', color: '#991b1b', padding: '10px', borderRadius: '8px', marginBottom: '20px', fontSize:'0.85rem', border:'1px solid #fecaca'}}>
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={handleLogin} style={{textAlign: 'left'}}>
                    ...
                    <div style={{marginBottom: 30}}>
                        <label style={{display: 'block', marginBottom: 8, fontSize: '0.85rem', fontWeight: 600, color: '#334155'}}>Password</label>
                        <div style={{position: 'relative'}}>
                    ...
                    {/* TOMBOL LOGIN (Disesuaikan untuk Loading) */}
